 ...
 *
 * Effects:
 * - ON_TAME: CONDITIONAL_AREA - Earn 2 points for each card with cost ≤ 2 in your area
 *
 * @version 1.0.0
 */
...
test.describe(`${CARD_ID} - ${CARD_NAME_TW} - ON_TAME Test`, () => {
  test('ON_TAME: Earn 2 points per cost≤2 card', async ({ page }) => {
    ...
    // PHASE 4: Setup field with low-cost cards
    console.log('\n[TEST] PHASE 4: Setting up field with low-cost cards (cost ≤ 2)')
    await openDevTestPanel(page)

    // Tame F002 (cost 1) - should count
    await directlyTameCard(page, 'F002', '小惡魔')
    await page.waitForTimeout(1000)

    // Tame another F002 (cost 1) - should count
    await directlyTameCard(page, 'F002', '小惡魔')
    await page.waitForTimeout(1000)

    console.log('[TEST] Field setup complete: 2x F002 (cost 1)')

    // PHASE 5: Tame F008 and verify effect
    ...
    // Expected: Field has 3 cards (2x F002 + 1x F008), all have cost ≤ 2
    // F002: cost 1 (✅ counts)
    // F002: cost 1 (✅ counts)
    // F008: cost 2 (✅ counts)
    // Expected score gain: 3 cards × 2 points = 6 points

    const conditionalLog = consoleLogs.find(log =>
      log.includes('CONDITIONAL_AREA') &&
      (log.includes('cards') || log.includes('cost'))
    )
    ...
    console.log('[TEST] Conditional area log:', conditionalLog)
    console.log('[TEST] Score update log:', scoreUpdateLog)

    // Verify logs exist
    expect(conditionalLog).toBeTruthy()
    expect(scoreUpdateLog).toBeTruthy()

    // Parse the score change from log
    // Expected format: "CONDITIONAL_AREA: updated score X -> Y (+Z)"
    ...
    // Verify: F008 should give 2 points × 3 cards (cost ≤ 2) = 6 points
    expect(scoreChange).toBe(6)

    console.log('\n[TEST] ✅ F008 ON_TAME TEST PASSED')
    console.log('========================================')
    console.log(`ON_TAME: Earned ${scoreChange} points for 3 cards with cost ≤ 2 ✅`)
    console.log('Expected: 3 cards × 2 points = 6 points')
    console.log('========================================')
  })
})
